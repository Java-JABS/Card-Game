...
public class Game {
    ...
    private void newSet(Player ruler) {
        ...
        synchronized (this) {
            ...
            gameState = (teams[0].getSet() >= 1 || teams[1].getSet() >= 1) ? GameState.END : GameState.RULE;
            minorUpdate.setGameState(gameState);
    ...
    private Card getHighestCard(Collection<Card> onTableCards) {
        ...
        boolean isRulePlayed = false;
        for (Card card : onTableCards) {
            if (card.suit() == rule && !isRulePlayed) {
                highestCard = card;
                isRulePlayed = true;
    ...
    public void rule(Player player, CardsSuit rule) throws RequestException {
        synchronized (this) {
            if (gameState != GameState.RULE)
                throw new RequestException(RequestErrorMessage.NOT_RULE_STATE);
            if (!players.contains(player)) throw new RuntimeException();
    ...
    public GameUpdate getUpdate(Player player, boolean isMajorUpdate) {
        synchronized (this) {
            ...
            if (isMajorUpdate) {
                ...
                if ((player == ruler && (previousGameState == GameState.RULE || gameState == GameState.RULE)) || previousGameState == GameState.RULE)
                    gameUpdate.setDast(player.dast);
            }
            notify();
            return gameUpdate;
