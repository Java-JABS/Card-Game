...
import rule.GameUpdate;
import rule.client.ClientRequestSender;
import rule.messages.*;
import rule.server.GameState;
import rule.server.Team;
...
public class OuterGamePanel extends JPanel {
    ...
    OuterGamePanel(OuterGameLabel topPanel) {
        ...
        JLabel ruleIconLabel = new JLabel();
        ruleIconLabel.setForeground(Color.BLACK);
        ruleIconLabel.setBorder(BorderFactory.createLineBorder(Color.BLACK));
        ruleIconLabel.setPreferredSize(new Dimension(78, 78));
        GridBagConstraints ruleIconLabelGrid = new GridBagConstraints();
        ruleIconLabelGrid.gridx = 2;
        ruleIconLabelGrid.gridy = 0;
        ruleIconLabelGrid.insets = new Insets(0, 50, 0, 0);
        upperPanel.add(ruleIconLabel, ruleIconLabelGrid);
                    ...
                    try {
                        ...
                        switch (newGameUpdate.getNumber() - gameUpdate.getNumber()) {
                            ...
                            case 1:
                                ...
                                if (newGameUpdate.getGameState() == GameState.NEW_SET || (lastGameUpdate.getGameState() == GameState.RULE && lastGameUpdate.getCurrentRuler() == lastGameUpdate.getYourIndex())) {
                                ...
                                if (gameUpdate.getGameState() == GameState.RULE && gameUpdate.getYourIndex() == gameUpdate.getCurrentRuler()) {
                                    ...
                                    topFrame.client.sendMessage(new RuleRequest(buttons[returnValue]));
                                }
